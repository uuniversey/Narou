...
function UserFollowingList() {
  ...
  const { followList, getUserData } = userProfileStore()
  ...
  return (
    <>
      <ProfileGrid item xs={9}>
        ...
        <div>
          <ImageList>
            {followList.slice(currentPage * 8 - 8, currentPage * 8).map((item, index) => (
        ...
        <br />
        <PaginationStyle color="primary" count={Math.floor(followList.length / 8) + 1} page={currentPage} onChange={handlePageNumber} />
